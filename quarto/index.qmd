...
date: "`r Sys.Date()`"
categories: [Syntopy, Simulated Data, R, Bayesian Inference]
format: html
editor: visual
...
For Fisher's non-central hypergeometric distribution we can use the library `BiasedUrn`, and for the modelling, we will use libraries `rethinking` and `rstan`. For data wrangling we will use library `dplyr`.
